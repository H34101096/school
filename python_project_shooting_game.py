...
pygame.display.set_caption("Shooting Game")   #遊戲標題
...
def draw_text(surf , text , size , x , y):
  ...
  text_surface = font.render(text , True , WHITE)   #設置文字內容和顏色(白色)
...
def draw_health(surf , health_point , x , y):
  ...
  pygame.draw.rect(surf , YELLOW , fill_rect)    #內部填滿
  pygame.draw.rect(surf , WHITE , outline_rect , 2)   #外框

#畫出初始畫面
def draw_inti():
  draw_text(screen , 'Shooting Game' , 68 , WIDTH*0.5 , HEIGHT*0.25)     #標題
  draw_text(screen , 'Press space and arrow key to shoot and move.' , 24 , WIDTH*0.5 , HEIGHT*0.5)   #說明遊戲遊玩方法
  draw_text(screen , 'Press any key to start' , 26 , WIDTH/2 , HEIGHT*0.75)      #遊戲開始方式
...
class Bullet(pygame.sprite.Sprite):
  ...
  def __init__(self,x,y):
    ...
    self.image.fill(YELLOW)      #子彈顏色為黃色(255,255,0)
    self.rect = self.image.get_rect()
...
while running:
  ...
  draw_health(screen , player.health*20 , 5 , 13)     #顯示生命線
  pygame.display.update()
